...
**MULTI-AGENT PROCESSING MODEL**: This system supports concurrent processing by multiple agents using isolated anime-specific directories to prevent conflicts and enable scalability.

1.  **Agent Slot Detection:**
    - Scan for existing agent directories in `temp/` directory: `temp/<anime_title>_agent1/`, `temp/<anime_title>_agent2/`, etc.
    - Determine next available agent slot (e.g., if `temp/One_agent1/` and `temp/Dandadan_agent2/` exist, create `temp/<new_anime>_agent3/`)
...
2.  **Load Source Database:**
    ...
    - Skip entries that match any currently processing anime (check against all existing agent directories)

3.  **Prepare Current Anime (Agent-Specific):**
    - Create agent-specific directory: `temp/<first word from anime title>_agent<N>/`
    - Extract the first entry from the `data` array and save it as `current_anime.json` in your agent directory
    - Add `processing: true` to the entry to indicate this agent is currently processing it
    - This `current_anime.json` file becomes the `offline_anime_data` for all subsequent processing steps
    - Each agent has its own isolated directory, enabling parallel processing without conflicts
    - The file enables manual review and debugging if needed

4.  **Verification:**
    - Confirm your agent's `current_anime.json` file contains a valid anime object
    ...
    - Your agent directory structure: `temp/<first word from anime title>_agent<N>/current_anime.json`
...
1.  **Jikan Anime Full Data:** Fetch full anime data from Jikan API using the MAL ID. Save in your agent directory: `temp/<first word from anime title>_agent<N>/jikan.json`
    - URL: `https://api.jikan.moe/v4/anime/{mal_id}/full`
2.  **Jikan Episodes Data:** Fetch episode data from Jikan API. Use episodes property from the `offline_anime_data` (from current_anime.json). Do not skip any episode. Save in your agent directory: `temp/<first word from anime title>_agent<N>/episodes.json`
    - **FIRST:** Create `temp/<first word from anime title>_agent<N>/episodes.json` with the episode count from offline_anime_data: `{"episodes": <episode_count>}`
    - **IMPORTANT:** For anime with >100 episodes, use the reusable script:
      `python src/enrichment/api_helpers/jikan_helper.py episodes {mal_id} temp/<first word from anime title>_agent<N>/episodes.json temp/<first word from anime title>_agent<N>/episodes_detailed.json`
    - **CRITICAL:**
      ...
      - The reusable script will read the episode count from your agent directory and fetch detailed data for each episode from the Jikan API endpoints
    - URL: `https://api.jikan.moe/v4/anime/{mal_id}/episodes/{episode_num}`
3.  **Jikan Characters Data:** Fetch character data from Jikan API. Do not skip any character. Save in your agent directory: `temp/<first word from anime title>_agent<N>/characters.json`
    - **IMPORTANT:** For anime with >50 characters, use the reusable script: `python src/enrichment/api_helpers/jikan_helper.py characters {mal_id} temp/<first word from anime title>_agent<N>/characters.json temp/<first word from anime title>_agent<N>/characters_detailed.json`
...
4.  **AnimSchedule Data:** Find a matching anime on AnimSchedule using REAL API calls. Save in your agent directory: `temp/<first word from anime title>_agent<N>/animeschedule.json`
...
5.  **Kitsu Data:** Fetch comprehensive Kitsu data using the extracted Kitsu ID. Save in your agent directory: `temp/<first word from anime title>_agent<N>/kitsu.json`
...
6.  **Anime-Planet Data:** Fetch comprehensive Anime-Planet data using web scraping. Save in your agent directory: `temp/<first word from anime title>_agent<N>/anime_planet.json`
...
7.  **AniList Data:** Fetch comprehensive AniList data using the extracted AniList ID. Save in your agent directory: `temp/<first word from anime title>_agent<N>/anilist.json`
...
8.  **AniDB Data:** Fetch comprehensive AniDB data using the extracted AniDB ID. Save in your agent directory: `temp/<first word from anime title>_agent<N>/anidb.json`
    - **IMPORTANT:**
...
1.  **Stage 1: Metadata Extraction** PROMPT: src/enrichment/prompts/stages/01_metadata_extraction.txt
    ...
    - **Output:** `temp/<first word from anime title>_agent<N>/stage1_metadata.json`

**Agent 2 - Episode Specialist:** 2. **Stage 2: Episode Processing** PROMPT: src/enrichment/prompts/stages/02_episode_processing.txt - **Inputs:** The pre-processed episode list. - **Action:** Process episodes in batches. For each batch, generate a list of `episode_details`. DO NOT skip any episode - **Output:** `temp/<first word from anime title>_agent<N>/stage2_episodes.json`
...
4.  **Stage 4: Statistics** PROMPT: src/enrichment/prompts/stages/04_statistics_media.txt
    ...
    **CRITICAL RULES:**
    ...
    - Prioritize AniDB for comprehensive staff data merging including detailed roles and credits.
      **Output:** `temp/<first word from anime title>_agent<N>/stage4_statistics_media.json`

**Agent 4 - Character & Staff Specialist:** 5. **Stage 5: Character Processing** PROMPT: src/enrichment/prompts/stages/05_character_processing.txt - **Inputs:** Jikan characters data, AniList characters data. - **Action:** Process characters in batches. For each batch, generate a list of `characters`. DO NOT skip any character. Merge character data from multiple sources for comprehensive character profiles. - **Output:** `temp/<first word from anime title>_agent<N>/stage5_characters.json`
...
6.  **Stage 6: Staff Processing** PROMPT: src/enrichment/prompts/stages/06_staff_processing.txt
    ...
    - **Output:** `temp/<first word from anime title>_agent<N>/stage6_staff.json`
...
- `temp/<first word from anime title>_agent<N>/stage1_metadata.json`
- `temp/<first word from anime title>_agent<N>/stage2_episodes.json`
- `temp/<first word from anime title>_agent<N>/stage3_relationships.json`
- `temp/<first word from anime title>_agent<N>/stage4_statistics_media.json`
- `temp/<first word from anime title>_agent<N>/stage5_characters.json`
- `temp/<first word from anime title>_agent<N>/stage6_staff.json`
...
1.  **Synchronization Check:** Verify all 6 stage output files from the 4 agents exist before proceeding:
    - `temp/<first word from anime title>_agent<N>/stage1_metadata.json` (Agent 1)
    - `temp/<first word from anime title>_agent<N>/stage2_episodes.json` (Agent 2)
    - `temp/<first word from anime title>_agent<N>/stage3_relationships.json` (Agent 3)
    - `temp/<first word from anime title>_agent<N>/stage4_statistics_media.json` (Agent 3)
    - `temp/<first word from anime title>_agent<N>/stage5_characters.json` (Agent 4)
    - `temp/<first word from anime title>_agent<N>/stage6_staff.json` (Agent 4)
...
2.  **Update Source Database (Agent-Safe Removal):**
    ...
    - **CRITICAL**: Remove the processed anime entry by matching against the exact anime from your agent's `current_anime.json` file
...
3.  **Agent Cleanup (Multi-Agent Environment):**
    - Clean up your agent's entire temp directory (e.g., `temp/<anime_title>_agent<N>/`)
    - This removes `current_anime.json`, all API data files, and all stage output files
    - **Do NOT** remove other agents' temp directories
