...
class SearchRequest(BaseModel):
    ...
    limit: int = Field(default=20, ge=1, le=settings.max_search_limit, description="Maximum number of results")
...
class ImageSearchRequest(BaseModel):
    ...
    limit: int = Field(default=10, ge=1, le=settings.max_search_limit, description="Maximum number of results")
    use_hybrid_search: bool = Field(default=True, description="Use hybrid search combining picture and thumbnail vectors")
...
class MultimodalSearchRequest(BaseModel):
    ...
    limit: int = Field(default=10, ge=1, le=settings.max_search_limit, description="Maximum number of results")
    text_weight: float = Field(default=0.7, ge=0.0, le=1.0, description="Weight for text similarity (0.0-1.0)")
...
async def search_by_image(request: ImageSearchRequest):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Image search operation failed: {str(e)}")
...
async def search_multimodal(request: MultimodalSearchRequest):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Multimodal search operation failed: {str(e)}")
