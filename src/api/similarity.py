...
class SimilarityResponse(BaseModel):
    ...
    similar_anime: List[Dict[str, Any]] = Field(
        ..., description="Similar anime results"
    )
...
class BatchSimilarityRequest(BaseModel):
    ...
    anime_ids: List[str] = Field(
        ..., description="List of anime IDs to find similarities for"
    )
    limit: int = Field(
        default=5, ge=1, le=20, description="Maximum similar anime per reference"
    )
    similarity_type: str = Field(
        default="semantic",
        description="Type of similarity: semantic, visual, or vector",
    )
...
async def get_similar_anime(
    anime_id: str,
    limit: int = Query(
        default=10, ge=1, le=50, description="Maximum number of similar anime to return"
    ),
) -> SimilarityResponse:
    """
    ...
    try:
        ...
        if not results:
            ...
            if not reference:
                raise HTTPException(
                    status_code=404, detail=f"Reference anime not found: {anime_id}"
                )
...
async def get_visually_similar_anime(
    ...
    limit: int = Query(
        ...
        ge=1,
        le=50,
        description="Maximum number of visually similar anime to return",
    ),
) -> SimilarityResponse:
    """
...
async def get_vector_similar_anime(
    ...
    try:
        ...
        results = await qdrant_client.get_similar_anime(anime_id=anime_id, limit=limit)
        ...
        raise HTTPException(
            status_code=500, detail=f"Vector similarity search failed: {str(e)}"
        )
...
async def get_batch_similarity(request: BatchSimilarityRequest) -> Dict[str, Any]:
    ...
    try:
        ...
        if len(request.anime_ids) > 10:
            raise HTTPException(
                status_code=400, detail="Maximum 10 anime IDs allowed in batch request"
            )
        ...
        raise HTTPException(
            status_code=500, detail=f"Batch similarity search failed: {str(e)}"
        )
