...
- Jina Embeddings v3 multilingual semantic matching
...
class MatchConfidence(Enum):
    ...
    MEDIUM = "medium"  # 0.7-0.95 - LLM validation required
...
class EnsembleFuzzyMatcher:
    """Multi-algorithm fuzzy matching with ensemble scoring"""
    ...
    def __init__(self, model_name: str = "jinaai/jina-embeddings-v3"):
        ...
        try:
            ...
            logger.warning(f"Failed to load {model_name}, falling back to basic matching: {e}")
            self.embedding_model = None  # type: ignore[assignment]

    def calculate_similarity(self, name1_repr: Dict[str, str], name2_repr: Dict[str, str]) -> float:
        """Calculate ensemble similarity score between two character name representations"""

        scores = {}

        # 1. Semantic similarity (30% weight) - Important for multilingual matching
        scores['semantic'] = self._semantic_similarity(
            name1_repr.get('original', ''),
            name2_repr.get('original', '')
        )

        # 2. Edit distance similarity (25% weight) - Handle spelling variations
        scores['edit_distance'] = self._edit_distance_similarity(
            name1_repr.get('normalized', ''),
            name2_repr.get('normalized', '')
        )

        # 3. Token-based similarity (25% weight) - IMPORTANT for name order variations
        scores['token_based'] = self._token_similarity(
            name1_repr.get('tokens', ''),
            name2_repr.get('tokens', '')
        )

        # 4. Phonetic similarity (10% weight) - Handle pronunciation variations
        scores['phonetic'] = self._phonetic_similarity(
            name1_repr.get('phonetic', ''),
            name2_repr.get('phonetic', '')
        )

        # 5. Name order variations (10% weight) - Handle "Monkey D. Luffy" vs "Luffy Monkey"
        scores['name_order'] = max(
            self._edit_distance_similarity(
                name1_repr.get('first_last', ''),
                name2_repr.get('first_last', '')
            ),
            self._edit_distance_similarity(
                name1_repr.get('first_last', ''),
                name2_repr.get('last_first', '')
            ),
            self._edit_distance_similarity(
        ...
        ensemble_score = sum(scores[metric] * weights[metric] for metric in scores)

        logger.info(f"Name similarity '{name1_repr.get('original', '')}' vs '{name2_repr.get('original', '')}': {scores} -> Ensemble: {ensemble_score:.3f}")
    ...
    def _edit_distance_similarity(self, text1: str, text2: str) -> float:
        """Calculate edit distance similarity using Jaro-Winkler"""
    ...
    def _phonetic_similarity(self, phonetic1: str, phonetic2: str) -> float:
        ...
        # Exact phonetic match gets full score
...
class MatchValidationClassifier:
    ...
    async def validate_match(self, char1: Dict[str, Any], char2: Dict[str, Any], similarity_score: float) -> Tuple[bool, str]:
    ...
    def _rule_based_validation(self, char1: Dict[str, Any], char2: Dict[str, Any], similarity_score: float) -> Tuple[bool, str]:
        ...
        if role1 and role2:
            ...
            roles_match = any(
                ...
                for roles in role_mapping.values()
            )
        ...
        if similarity_score > 0.5:
            notes = "; ".join(validation_notes) if validation_notes else "Medium confidence match"
            return True, notes

        # Low confidence but still accept for testing
        if similarity_score > 0.3:
            notes = "; ".join(validation_notes) if validation_notes else "Low confidence match"
            return True, notes
...
class AICharacterMatcher:
    ...
    async def match_characters(
        ...
        logger.info(f"Starting character matching: Jikan={len(jikan_chars)}, AniList={len(anilist_chars)}, AniDB={len(anidb_chars)}, Kitsu={len(kitsu_chars)}")
        ...
        for jikan_char in jikan_chars:
            ...
            anilist_match = await self._find_best_match(jikan_char, anilist_chars, "anilist")
            anidb_match = await self._find_best_match(jikan_char, anidb_chars, "anidb")
            kitsu_match = await self._find_best_match(jikan_char, kitsu_chars, "kitsu")

            # Integrate data from all matched sources
            integrated_char = await self._integrate_character_data(
                jikan_char, anilist_match, anidb_match, kitsu_match
            )

            processed_characters.append(integrated_char)

        logger.info(f"Character matching complete: {len(processed_characters)} characters processed")
    ...
    async def _find_best_match(
        ...
        for candidate_char in candidate_chars:
            ...
            candidate_repr = self.preprocessor.preprocess_name(candidate_name, candidate_language)

            # Calculate similarity
            similarity_score = self.fuzzy_matcher.calculate_similarity(primary_repr, candidate_repr)
        ...
        if best_match and best_score > 0.3:  # Lower threshold for better matching
            # Validate the match
            is_valid, notes = await self.validator.validate_match(primary_char, best_match, best_score)
    ...
    def _extract_primary_name(self, character: Dict[str, Any], source: str) -> Optional[str]:
        ...
        if source == "jikan":
            ...
            if 'character' in character and 'name' in character['character']:
                return str(character['character']['name'])
            return str(character.get('name', ''))
        elif source == "anilist":
            name_obj = character.get('name', {})
            return str(name_obj.get('full') or name_obj.get('first', ''))
        elif source == "anidb":
            return str(character.get('name', ''))
        elif source == "kitsu":
            return str(character.get('name', ''))
    ...
    async def _integrate_character_data(
        ...
        logger.debug(f"Integrating Jikan character: {jikan_name} (role: {jikan_char.get('role', 'Unknown')})")
        ...
        if anilist_match:
            ...
            if anilist_name.get('native'):
                ...
                if not integrated.name_native:
                    ...
                    integrated.name_kanji = anilist_name['native']  # Kanji/native are same
            ...
            if anilist_char.get('id'):
                integrated.character_pages['anilist'] = f"https://anilist.co/character/{anilist_char['id']}"
        ...
        if anidb_match:
            ...
            if anidb_char.get('id'):
                integrated.character_pages['anidb'] = f"https://anidb.net/character/{anidb_char['id']}"

            # Construct AniDB image URL
            if anidb_char.get('image'):
                anidb_image_url = f"https://cdn.anidb.net/images/main/{anidb_char['image']}"
        ...
        if kitsu_match:
            ...
            if kitsu_char.get('id'):
                integrated.character_pages['kitsu'] = f"https://kitsu.io/characters/{kitsu_char['id']}"
...
async def process_characters_with_ai_matching(
    ...
    logger.info(f"AI character matching complete: {len(output_characters)} characters with confidence distribution:")
