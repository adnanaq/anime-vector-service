...
class AniDBUDPHelper:
    ...
    def _extract_character_ids_from_response(self, parts: List[str]) -> List[int]:
        ...
        for part in parts:
            if ',' in part and part.replace(',', '').replace(' ', '').isdigit():
                try:
                    char_ids = [int(x.strip()) for x in part.split(',') if x.strip().isdigit()]
                    if char_ids:
    ...
    def get_all_characters_for_anime(self, anidb_id: int) -> Optional[List[Dict[str, Any]]]:
...
async def main():
    ...
    parser.add_argument("--output", type=str, default="test_anidb_udp_characters.json", help="Output file path")
    ...
    try:
        ...
        if characters:
            ...
            logger.info(f"Successfully saved {len(characters)} characters to {args.output}")
        else:
