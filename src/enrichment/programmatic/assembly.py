...
class EnrichmentAssembler:
    ...
    def _merge_programmatic_data(self, entry: Dict[str, Any], programmatic_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _merge_ai_stages(self, entry: Dict[str, Any], stage_outputs: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _merge_stage1_metadata(self, entry: Dict[str, Any], stage1_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        scalar_fields = [
            'background', 'episodes', 'month', 'nsfw', 'rating', 
            'source_material', 'status', 'synopsis', 'title', 
        ...
        # Array fields
        array_fields = [
            'content_warnings', 'demographics', 'genres', 'synonyms', 'themes', 'trailers'
        ]
        ...
        # Object fields
        object_fields = [
            'aired_dates', 'anime_season', 'broadcast', 'broadcast_schedule',
            'delay_information', 'duration', 'episode_overrides', 'external_links'
        ]
    ...
    def _merge_stage2_episodes(self, entry: Dict[str, Any], stage2_data: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        if not entry.get('episode_details'):
            ...
            existing_episodes = {ep.get('episode_number'): ep for ep in entry['episode_details']}
    ...
    def _merge_stage3_relationships(self, entry: Dict[str, Any], stage3_data: Dict[str, Any]) -> Dict[str, Any]:
        """Merge Stage 3: Relationships"""
        if not stage3_data:
            return entry
            
        if 'related_anime' in stage3_data and isinstance(stage3_data['related_anime'], list):
    ...
    def _merge_stage4_statistics(self, entry: Dict[str, Any], stage4_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _merge_stage5_characters(self, entry: Dict[str, Any], stage5_data: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    def _merge_stage6_staff(self, entry: Dict[str, Any], stage6_data: Dict[str, Any]) -> Dict[str, Any]:
        """Merge Stage 6: Staff"""
        if not stage6_data:
            return entry
    ...
    def _apply_schema_ordering(self, entry: Dict[str, Any]) -> Dict[str, Any]:
        ...
        SCALAR_FIELDS = [
            'background', 'episodes', 'month', 'nsfw', 'picture', 'rating', 
            'source_material', 'status', 'synopsis', 'thumbnail', 'title', 
            'title_english', 'title_japanese', 'type'
        ]
        
        ARRAY_FIELDS = [
            'awards', 'characters', 'content_warnings', 'demographics', 
            'ending_themes', 'episode_details', 'genres', 'licensors', 
            'opening_themes', 'related_anime', 'relations', 'sources', 
            'streaming_info', 'synonyms', 'tags', 'themes', 'trailers'
        ]
        
        OBJECT_FIELDS = [
            'aired_dates', 'anime_season', 'broadcast', 'broadcast_schedule', 
            'delay_information', 'duration', 'external_links', 'images', 
            'popularity_trends', 'premiere_dates', 'score', 'staff_data', 'statistics'
        ]
        ...
        try:
            ...
            for issue in validation_result.issues:
                if issue.severity == 'error':
                    self.errors.append(f"Final validation error in {issue.field_path}: {issue.description}")
                elif issue.severity == 'warning':
                    self.warnings.append(f"Final validation warning in {issue.field_path}: {issue.description}")
                else:  # info
                    self.warnings.append(f"Final validation info in {issue.field_path}: {issue.description}")
...
def validate_and_fix_entry(anime_entry: Dict[str, Any]) -> Tuple[Dict[str, Any], bool, List[str]]:
    """
    ...
    try:
        ...
        messages = []
        for issue in validation_result.issues:
            messages.append(f"{issue.severity.upper()}: {issue.field_path} - {issue.description}")
