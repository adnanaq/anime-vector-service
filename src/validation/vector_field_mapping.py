...
VECTOR_FIELD_MAPPINGS: Dict[str, Dict[str, List[str]]] = {
    ...
    "title_vector": {
        "fields": ["title", "title_english", "title_japanese", "synopsis", "background", "synonyms"],
...
DUAL_INDEXED_FIELDS = [
    "title", "genres", "tags", "demographics", "type", "status"
]

# Payload-only fields (precise filtering, no semantic search)
PAYLOAD_ONLY_FIELDS = [
    "id", "episodes", "rating", "nsfw", "anime_season", "duration",
    "sources", "statistics", "score"
]

# Non-indexed payload (storage only)
NON_INDEXED_PAYLOAD = [
    "enrichment_metadata", "images"
]
...
def get_vector_description(vector_name: str) -> str:
    ...
    return VECTOR_FIELD_MAPPINGS.get(vector_name, {}).get("description", "Unknown vector")

def get_text_vectors() -> List[str]:
    """Get list of all text vector names."""
    return [name for name in VECTOR_FIELD_MAPPINGS.keys()
