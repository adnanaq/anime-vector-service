...
try:
    ...
    print("Warning: Could not import Pydantic models. Schema validation will be limited.")
...
class EnrichmentValidator:
    ...
    REQUIRED_FIELDS = {
        ...
        'episode_details', 'statistics', 'enrichment_metadata'
    }
    
    # Collections that should be OMITTED when empty
    OMIT_EMPTY_COLLECTIONS = {
        'characters', 'awards', 'themes', 'genres', 'demographics',
        'streaming_info', 'opening_themes', 'ending_themes', 
        'relations', 'related_anime', 'content_warnings', 
        'licensors', 'synonyms', 'tags', 'trailers'
    }
    
    # Objects that should be OMITTED when empty
    OMIT_EMPTY_OBJECTS = {
        'images', 'external_links', 'staff_data', 'aired_dates',
        'broadcast', 'broadcast_schedule', 'anime_season', 'duration', 
        'premiere_dates', 'score', 'delay_information', 'episode_overrides', 
        'popularity_trends'
    }
    
    # Scalar fields that should be OMITTED when null/empty
    OMIT_IF_NULL = {
        'synopsis', 'picture', 'thumbnail', 'title_english', 
        'title_japanese', 'rating', 'source_material', 
    ...
    def check_required_fields(self, entry: Dict[str, Any]) -> List[ValidationIssue]:
        ...
        for field in self.REQUIRED_FIELDS:
            ...
            elif entry[field] is None and field not in ['statistics', 'enrichment_metadata']:
                issues.append(ValidationIssue(
    ...
    def check_empty_field_compliance(self, entry: Dict[str, Any]) -> List[ValidationIssue]:
    ...
    def _check_character_empty_fields(self, char: Dict[str, Any], char_index: int) -> List[ValidationIssue]:
        ...
        char_empty_scalars = {'name_kanji', 'name_native', 'description', 'age', 'gender'}
        
        # Check empty collections
        for field in char_empty_collections:
            if field in char and isinstance(char[field], list) and len(char[field]) == 0:
                issues.append(ValidationIssue(
        ...
        for field in char_empty_objects:
            if field in char and isinstance(char[field], dict) and self._is_empty_object(char[field]):
                issues.append(ValidationIssue(
    ...
    def _check_staff_data_empty_fields(self, staff_data: Dict[str, Any]) -> List[ValidationIssue]:
        ...
        for field in staff_empty_collections:
            if field in staff_data and isinstance(staff_data[field], list) and len(staff_data[field]) == 0:
                issues.append(ValidationIssue(
        ...
        if 'production_staff' in staff_data and isinstance(staff_data['production_staff'], dict):
            production_staff = staff_data['production_staff']
            
            # All possible production staff roles that could be empty
            production_roles = {
                'directors', 'music_composers', 'character_designers', 'series_writers',
                'animation_directors', 'original_creators', 'art_directors', 'sound_directors',
                'producers', 'writers', 'composers', 'designers', 'animators'
            }
            
            for role in production_roles:
                if role in production_staff and isinstance(production_staff[role], list) and len(production_staff[role]) == 0:
                    issues.append(ValidationIssue(
                ...
                if 'voice_actors' in char:
                    ...
                    if isinstance(va_list, list) and len(va_list) == 0:
                        ...
                        for j, va in enumerate(va_list):
                            if not isinstance(va, dict) or 'name' not in va or 'language' not in va:
                                issues.append(ValidationIssue(
    ...
    def validate_schema_compliance(self, entry: Dict[str, Any]) -> List[ValidationIssue]:
    ...
    def generate_report(self) -> Dict[str, Any]:
        ...
        report = {
            'validation_summary': {
                ...
                'compliance_percentage': round((valid_entries / total_entries) * 100, 2),
            ...
            'most_problematic_entries': [
                ...
                for entry in problematic_entries if entry.error_count > 0
            ],
            'detailed_results': [
                {
    ...
    def auto_fix_entry(self, entry: Dict[str, Any]) -> Tuple[Dict[str, Any], List[str]]:
        ...
        if 'staff_data' in fixed_entry and 'voice_actors' in fixed_entry['staff_data']:
            ...
            if isinstance(va_data, list):
                if len(va_data) == 0:
                    ...
                    fixes_applied.append("Removed empty voice_actors array from staff_data")
                else:
                    # Convert to proper structure
                    fixed_entry['staff_data']['voice_actors'] = {'japanese': va_data}
                    fixes_applied.append("Converted voice_actors array to proper object structure")
            elif isinstance(va_data, dict) and self._is_empty_object(va_data):
                # Remove empty voice_actors object
                del fixed_entry['staff_data']['voice_actors']
                fixes_applied.append("Removed empty voice_actors object from staff_data")
        ...
        for field in self.OMIT_IF_NULL:
            if field in fixed_entry and (fixed_entry[field] is None or fixed_entry[field] == ""):
                del fixed_entry[field]
        ...
        if 'characters' in fixed_entry:
            for char in fixed_entry['characters']:
                ...
                if 'voice_actors' in char and isinstance(char['voice_actors'], list) and len(char['voice_actors']) == 0:
                    del char['voice_actors']
                    fixes_applied.append(f"Removed empty voice_actors from character: {char_name}")
                
                # Remove empty collections
                for field in ['name_variations', 'nicknames']:
                    if field in char and isinstance(char[field], list) and len(char[field]) == 0:
                        del char[field]
                        fixes_applied.append(f"Removed empty {field} from character: {char_name}")
                ...
                for field in ['name_kanji', 'name_native', 'description', 'age', 'gender']:
                    if field in char and (char[field] is None or char[field] == ""):
                        del char[field]
                        fixes_applied.append(f"Removed null/empty {field} from character: {char_name}")
...
def main():
    ...
    parser.add_argument('--file', default='data/enriched_anime_database.json', 
    ...
    if report['most_problematic_entries']:
        ...
        for entry in report['most_problematic_entries'][:3]:
            ...
            print(f"    Errors: {entry['error_count']}, Warnings: {entry['warning_count']}")
